        ...
        @foreach ($holidays as $i => $holiday)
            <div class="mb-3">
                <div class="w-100">
                    ...
                    <div class="mb-3">
                        <x-form-label id="holiday_date{{ $i }}"
                            label='Tanggal Hari Libur ({{ $i + 1 }})' />
                        <x-form-input type="date" id="holiday_date{{ $i }}"
                            name="holiday_date{{ $i }}" class="form-control"
                ...
                @if ($i > 0)
                    <button class="btn btn-sm btn-danger mt-2" wire:click="removeHolidayInput({{ $i }})"
                        wire:target="removeHolidayInput({{ $i }})" type="button"
